...
class CNN():
  ...
  def getData(self, image_path, val):
    ...
    # data, = shuffle(img_matrix, img_label, random_state = 2)
    # self.train_image = [img_matrix,img_label]
    
    if val == 0:
      self.train_image = np.reshape(img_matrix, (32, 32, 3))
      ...
      self.test_images = np.reshape(img_matrix, (32, 32, 3))
